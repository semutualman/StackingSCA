...
def load_ascad(ascad_database_file, load_metadata=True):
    ...
    if load_metadata == False:
        return (X_profiling, Y_profiling), (X_attack, Y_attack),(Y_attack,Y_attack)
    else:
...
def Stacking(X_profiling, Y_profiling, save_file_name):
    ...
    d = np.hstack((X[:, 46510:46610], X[:, 47200:47400], X[:, 54200:54400], X[:, 71600:71700], X[:, 71900:72000]))
    # X_profiling=np.hstack((X_profiling, X[:, 47250:47260]))
    ...
    model2 = run_cnn(second_level_train_set[:40000], Y_profiling[:40000], second_level_train_set[40000:],Y_profiling[40000:])
    test_nfolds_sets = test_nfolds_sets.reshape([10000, 256])
    predictions=model1.predict(test_nfolds_sets)
    predictions1 = model2.predict(test_nfolds_sets.reshape((test_nfolds_sets.shape[0], test_nfolds_sets.shape[1], 1)))
    d_in='D:/Methodology-for-efficient-CNN-architectures-in-SCA-master/ASCAD/N0=0/ASCAD_dataset/second_dataset/'
    np.save(d_in + "predictions.npy", predictions)

    return predictions

def Stacking1(X_profiling, Y_profiling, save_file_name):
    second_level_train_set = np.zeros((50000, 256))
    second_level_test_set = np.zeros((10, 10000, 256))
    test_nfolds_sets = np.zeros((1,10000,256))
    ...
    predictions1 = model2.predict(test_nfolds_sets.reshape((test_nfolds_sets.shape[0], test_nfolds_sets.shape[1], 1)))
    d_in='D:/Methodology-for-efficient-CNN-architectures-in-SCA-master/ASCAD/N0=0/ASCAD_dataset/second_dataset/'
    np.save(d_in+"predict1.npy",predictions)
...
# d_in = 'D:/Methodology-for-efficient-CNN-architectures-in-SCA-master/ASCAD/N0=0/ASCAD_dataset/second_dataset/'
# np.save(d_in + "second_train4.npy", meta_train)
# # np.save(d_in + "Y_profiling.npy", Y_profiling)
# np.save(d_in + "second_test4.npy", meta_test)
...
np.save(predictions_folder + 'predictions' + model_name + '.npy', predictions)

# predictions=np.load(d_in+"predict1.npy")
